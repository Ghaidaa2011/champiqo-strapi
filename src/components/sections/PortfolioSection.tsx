...
const PortfolioSection = () => {
  ...
  useEffect(() => {
    const fetchSpecialists = async () => {
      try {
        const response = await fetch(`${BASE_URL}/api/specialists?populate=*`);
  ...
  return (
    <section id="specialists" className="section bg-secondary/30 min-h-[50vh]">
      <div className="container">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold mb-4">
            {t("specialists.title", "Our Specialists")}
          </h2>

          <p className="text-lg text-foreground/70">
            {t("specialists.subtitle", "Meet our team of experts")}
          </p>
        </motion.div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {specialists.map((specialist, index) => {
              const imageUrl = specialist?.image?.url
                ? `${BASE_URL}${specialist.image.url}`
